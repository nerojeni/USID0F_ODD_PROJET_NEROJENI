...
from projet_sante_mentale.config.settings import (
    DATA_URL,
    PATH_FILE,
    TEMPLATE_DIR,
    PATH_FILE,
...
def init_workbook(df: pd.DataFrame) -> None:
    """
    Supprime le fichier Excel existant s'il y en a un, puis le recrée
    de zéro avec uniquement la feuille DATA.

    Repartir d'un fichier vide à chaque exécution évite toute pollution
    par des feuilles, Tables ou ArrayFormulas issues d'exécutions
    précédentes (ex. ValueError "Table with name tblGenres already exists").
    """
    os.makedirs(os.path.dirname(PATH_FILE), exist_ok=True)

    # Repartir d'un fichier propre
    ...
    # Le moteur 'openpyxl' est forcé : c'est le seul moteur pandas
    # supportant mode="a" (xlsxwriter, s'il est installé, lève
    # "Append mode is not supported with xlsxwriter!").
    with pd.ExcelWriter(PATH_FILE, engine="openpyxl",
